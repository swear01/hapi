...
Release-manifest detection resolves physical paths so macOS `/var` and
`/private/var` aliases cannot bypass audit or header-pin enforcement.

## Rehearsal

Pin the audited upstream SHA. That SHA must equal `upstream/main`, or be an
ancestor of `upstream/main` (latest GitHub Release while main has moved on).
A pin that is not on the official main line fails:
