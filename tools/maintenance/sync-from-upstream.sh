...
if [ -n "$manifest_upstream_sha" ] && [ "$manifest_upstream_sha" != "$upstream_sha" ]; then
    echo "MANIFEST_UPSTREAM_SHA_MISMATCH expected=$manifest_upstream_sha actual=$upstream_sha" >&2
    exit 4
fi
