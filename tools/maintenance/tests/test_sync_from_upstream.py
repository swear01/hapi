...
class SyncFromUpstreamTest(unittest.TestCase):
    ...
    def test_accepts_upstream_pin_that_is_ancestor_of_main(self):
        pin = self.upstream_sha
        run('git', '-C', str(self.seed), 'reset', '--hard', 'upstream/main')
        (self.seed / 'later.txt').write_text('later official\n')
        run('git', '-C', str(self.seed), 'add', 'later.txt')
        run('git', '-C', str(self.seed), 'commit', '-m', 'later official')
        run('git', '-C', str(self.seed), 'push', 'upstream', 'HEAD:main')

        env = os.environ | {'HAPI_UPSTREAM_SHA': pin}
        ...
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertIn('UPSTREAM_PIN_ANCESTOR', result.stdout)
        self.assertIn('REHEARSAL_OK', result.stdout)
