...
describe('Codex conversation-history hub integration', () => {
    it('forwards serialized Fork RPC errors instead of using the missing-id fallback', async () => {
        ...
        try {
            engine.getOrCreateMachine('machine-1', { host: 'localhost', platform: 'linux', happyCliVersion: 'test', homeDir: '/tmp', capabilities: [MACHINE_CAPABILITIES.SessionControlSkill] }, null, 'default')
