...
describe('Store V28→V29 migration: ACP usage columns', () => {
    it('upgrades a base V28 database before the usage scan', () => {
        const directory = mkdtempSync(join(tmpdir(), 'hapi-migration-v28-to-v29-'))
        ...
        try {
            ...
            const columns = (table: string) => new Set(
                (internalDb.prepare(`PRAGMA table_info(${table})`).all() as Array<{ name: string }>)
                    .map((column) => column.name)
            )
            const usageColumns = columns('usage_events')
            const version = internalDb.prepare('PRAGMA user_version').get() as { user_version: number }

            expect(version.user_version).toBe(29)
            expect(columns('fcm_devices').has('push_key')).toBe(true)
            expect(columns('messages').has('delivery_state')).toBe(true)
    ...
    it('rebuilds stale usage indexes when a v28 database already has the new columns', () => {
        const directory = mkdtempSync(join(tmpdir(), 'hapi-migration-v28-recovery-'))
        const dbPath = join(directory, 'test.db')
        let store: Store | undefined
        try {
            store = new Store(dbPath)
