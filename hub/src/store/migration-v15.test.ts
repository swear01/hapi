...
describe('Store V14→V15 migration: scratchlist attachments column', () => {
    ...
    it('fork-polluted schema 16 (already bumped, still no attachments) repairs on reopen', () => {
        const dir = mkdtempSync(join(tmpdir(), 'hapi-migration-fork-v16-pollution-'))
        const dbPath = join(dir, 'test.db')
        let store: Store | undefined
        try {
            const db = new Database(dbPath, { create: true, readwrite: true, strict: true })
 ...
 * builds may bump user_version to 16 without repairing DDL.
 */
