...
export function useDictation(config: {
            ...
            recorder.onstop = async () => {
                ...
                try {
                    ...
                    try {
                        ...
                        success = transcribedText.trim().length > 0
                        if (pendingSend) {
    ...
    const stop = useCallback(async (waitForTranscript = true): Promise<boolean> => {
        ...
        if (recorder && recorder.state !== 'inactive') {
            transcribingRef.current = true
            setStatus('connecting')
            const completionPromise = waitForTranscript ? new Promise<boolean>((resolve) => {
                stopResolverRef.current = resolve
            }) : null
            recorder.stop()
            return completionPromise ? await completionPromise : false
        } else {
            setStatus('disconnected')
            stopTracks()
            return false
