...
import type { ClaudeImportResult, ClaudeLocalSessionSummary, CodexDuplicateSessionGroup, CodexLocalSessionSummary, Machine, PiImportResult, PiLocalSessionSummary } from '@/types/api'
...
import { MODEL_OPTIONS, type AgentType, type LaunchEffort, type CodexReasoningEffort, type NewSessionServiceTier, type SessionType } from './types'
...
export function NewSession(props: {
    ...
    const availableAgents = useMemo(
        () => agentAvailability.agents
            ...
            .filter((candidate) => visibleAgents.includes(candidate)),
    ...
    const loadClaudeImportSessions = useCallback(async () => {
        ...
        try {
            ...
            setSelectedClaudeImportSessionId((current) => current && result.sessions.some((session: ClaudeLocalSessionSummary) => session.id === current) ? current : null)
        } catch (loadError) {
            if (generation !== claudeLoadGenerationRef.current) return
            setClaudeImportSessions([])
            setClaudeImportMachineId(null)
    ...
    const handleBulkImportClaudeSessions = useCallback(async (sessionIds: string[]) => {
        ...
        try {
            ...
            const importedCount = result.results.filter((item: ClaudeImportResult) => item.hapiSessionId && !item.error).length
            const failed = result.results.filter((item: ClaudeImportResult) => item.error)
            if (importedCount > 0) {
                addToast({
    ...
    const handleBulkImportPiSessions = useCallback(async (sessionIds: string[]) => {
        ...
        try {
            ...
            const importedCount = result.results.filter((item: PiImportResult) => item.hapiSessionId && !item.error).length
            const failed = result.results.filter((item: PiImportResult) => item.error)
            if (importedCount > 0) {
                addToast({
    ...
    async function handleCreate() {
        ...
        try {
            ...
            if (agent === 'claude' && selectedClaudeImportSession) {
                ...
                const imported = result.results.find((item: ClaudeImportResult) => item.claudeSessionId === selectedClaudeImportSession.id)
                if (imported?.error) {
