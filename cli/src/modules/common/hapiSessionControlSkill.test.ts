...
describe('canonical hapi-session-runtime delivery', () => {
    let sandbox: string
    let workingDirectory: string
    let originalEnv: Partial<Record<typeof ENV_KEYS[number], string | undefined>>

    beforeEach(async () => {
        sandbox = await mkdtemp(join(tmpdir(), 'hapi-session-runtime-'))
    ...
    it('preserves an existing Skillshare session-control symlink', async () => {
        const sourceDir = join(sandbox, 'skillshare-session-control')
        const existingDir = join(nativeSkillRoot('codex'), 'hapi-session-control')
        await mkdir(sourceDir, { recursive: true })
        await writeFile(join(sourceDir, 'SKILL.md'), 'user-managed workflow')
        await mkdir(nativeSkillRoot('codex'), { recursive: true })
        await symlink(sourceDir, existingDir)

        const target = await ensureHapiSessionControlSkill('codex', workingDirectory)

        expect(target).not.toContain('hapi-session-control')
        expect((await lstat(existingDir)).isSymbolicLink()).toBe(true)
        expect(await readFile(join(sourceDir, 'SKILL.md'), 'utf8')).toBe('user-managed workflow')
    })
