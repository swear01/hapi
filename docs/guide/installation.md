...
Install and authenticate HAPI on the runner host. Session creation installs and verifies the bundled `hapi-session-runtime` skill there, where the agent actually executes.
...
[Runner host]  hapi runner start  → spawns session CLIs
                     │
                     ▼
              agent session  → native hapi-session-runtime skill
